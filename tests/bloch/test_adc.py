...
def test_observe(device, nlocations, npools):
    ...
    """
    # define
    w = [torch.as_tensor([1.0], dtype=torch.float32, device=device), torch.as_tensor([0.5, 0.5], dtype=torch.float32, device=device)]
    nstates = 2

    # initialize
    states = EPGstates(device, 1, nstates, nlocations, 1, npools, weight=w[npools-1])["states"]
