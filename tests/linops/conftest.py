...
class Helpers: # noqa
    ...
    def check_linop_unitary(A, data): # noqa
        ...
        AHA = A.H * A
        npt.assert_allclose(AHA(x).numpy(force=True), x.numpy(force=True), atol=atol, rtol=rtol)
    ...
    def check_linop_linear(A, data): # noqa
        ...
        npt.assert_allclose(
            A(a * x + y).numpy(force=True), (a * A(x) + A(y)).numpy(force=True), atol=atol, rtol=rtol
        )
    ...
    def check_linop_adjoint(A, idata, odata): # noqa
        ...
        lhs = _vdot(A(x), y)
        rhs = _vdot(A.H.H(x), y)
        npt.assert_allclose(lhs.numpy(force=True), rhs.numpy(force=True), atol=atol, rtol=rtol)
