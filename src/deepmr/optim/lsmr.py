...
class LSMRStep(nn.Module):
    ...
    Armat : callable, optional
        Function to compute the adjoint of A. Defaults to None.
    n : int, optional
        Dimension of the problem. If None, it is computed from A. Defaults to None.
    damp : float, optional
        Damping factor for regularized least-squares. Defaults to 0.0.
    atol : float, optional
        Absolute stopping tolerance. Defaults to 1e-6.
    btol : float, optional
        Relative stopping tolerance. Defaults to 1e-6.
    conlim : float, optional
        Maximum allowed condition number of A. Defaults to 1e8.
    maxiter : int, optional
        Maximum number of iterations. If None, set to min(m, n). Defaults to None.
    x0 : torch.Tensor, optional
        Initial guess for the solution. If None, zeros are used. Defaults to None.
    check_nonzero : bool, optional
        Flag to check if beta is nonzero. Defaults to True.
    ...
    def __init__(self, A, b, Armat=None, n=None, damp=0.0, atol=1e-6, btol=1e-6, conlim=1e8, maxiter=None, x0=None, check_nonzero=True):
        super().__init__()
        ...
        if torch.is_tensor(self.A):
            ...
            if self.Armat is None:
                self.Armat = (torch.adjoint(self.A)).matmul
            self.A = self.A.matmul
        elif not callable(self.A):
        ...
        if torch.is_tensor(self.Armat):
            self.Armat = self.Armat.matmul
        elif not callable(self.Armat):
            raise RuntimeError("matmul_closure must be a tensor, or a callable object!")
