...
def excitation_pulse(flip, dur, tb=4, dz=None, B0=None, gmax=None, smax=None, gdt=4.0): #, nbands=1):
    """
    ...
    if ptype == "spatially-selective" or ptype == "spectral-spatial":
        assert gmax is not None, "Please provide hardware limitations for spatially selective pulses"
        assert smax is not None, "Please provide hardware limitations for spatially selective pulses"
        assert gdt is not None, "Please provide hardware limitations for spatially selective pulses"
    
    if "spat" in ptype:
        assert dz is not None and dz > 0, "Please provide slice thickness for spatially selective pulses"
        
    if "freq" in ptype or "spectral" in ptype:
        assert B0 is not None and B0 > 0, "Please provide field strength for frequency selective pulses"
    
    # now design
    if ptype == "nonselective":
        rf, g, zprof = _design_nonselective(alpha, dur, tb, gdt) #, nbands)
    elif ptype == "frequency-selective":
        rf, g, zprof = _design_freqselective(alpha, dur, tb, B0, gmax, smax, gdt) #, nbands)
    elif ptype == "spatially-selective":
        rf, g, zprof = _design_spatselective(alpha, dur, tb, dz, gmax, smax, gdt) #, nbands)
    ...
    return {"flip": flip, "b1sqrdTau": b1sqrdTau, "zprof": zprof}, {"rf": rf, "grad": {"slice": g}, "t": t}

#%% local utils      
def _design_nonselective(flip, dur, tb, gdt, nbands=1, bandsep=None): 
    # design base pulse
    N = int(round(dur / gdt / 1e-6))
    rf0 = dzrf(n=N, tb=tb, ptype="st", ftype="min", d1=0.01, d2=0.01, cancel_alpha_phs=True)
...
def _design_spsp(flip, dur, tb, dz, B0, gmax, smax, gdt):
    ...
    B0 *= 1e4 # T -> G
    fspec = B0 * (np.asarray([(fat1-df), (fat2+df), (water-df), (water+df)]) - water) * gambar
    ...
    # actual design
    g, rf, _ = ss_design(dz * 1e2, tb, np.asarray([0.01, 0.01]), fspec, np.asarray([0.0, 1.0]) * flip, np.asarray([0.02, 0.005]), "ex", "ls", "min", "Flyback Half", f_off, sg=sg)
