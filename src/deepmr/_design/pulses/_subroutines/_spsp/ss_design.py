...
def ss_design(
    ...
    if sg.SS_NUM_LOBE_ITERS == 1:
        ...
        if rf is not None:
            ...
            if verbose:
                ...
                print(
                    f"Fs: {fs_best:6.1f} B1: {b1_best:5.3f}G Power: {pow_best:5.3e} G^2 ms Dur: {dur_best * 1e3:4.1f}ms"
                )
        else:
            if verbose:
        ...
        for band in range(nbands - 1, -1, -1):
            ...
            for iter in range(niter - 1, -1, -1):
                ...
                if len(rf) == 0:
                    if verbose:
                        print(
                            f"Band: {nbands - band}/{nbands} Iter: {niter - iter}/{niter} Fs: {fs:6.1f} *** No Soln ***"
                        )
                    continue
                ...
                infoall.append(
                    f"Fs: {fs:6.1f} B1: {b1:5.3f}G Power: {pow:5.3e} G^2 ms Dur: {dur * 1e3:4.1f}ms"
                )
                if verbose:
                    print(
                        f"Band: {nbands - band}/{nbands} Iter: {niter - iter}/{niter} {infoall[nsolutions]}"
                    )
    ...
    if len(rf) == 0:
        ...
        if len(Ifix) > 0:
            ...
            if verbose:
                print(
                    "Solution found by increasing ripples in bands %s"
                    % " ".join(map(str, Ifix))
                )
                print(
                    "Returning pulse with increased ripple in band %d (%.1f to %.1f Hz):"
                    % (Imin, f[2 * Imin - 2], f[2 * Imin - 1])
                )
            ...
            if verbose:
                print(
                    "Fs: %6.1f B1: %5.3fG Power: %5.3e G^2 ms Dur: %4.1fms"
                ...
                print(
                    "Increasing the max pulse duration or slice thickness may also help"
                )

        else:
            raise Exception(
                "No solution found! Try reducing bandwidths, increasing ripple, increasing max duration, increasing slice thickness..."
            )
