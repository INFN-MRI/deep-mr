...
def ss_flyback(ang, z_thk, z_tb, z_d, f, a, d, fs, ptype, z_ftype, s_ftype, ss_type, f_off, dbg, sg=None):    
    fs = float(fs)
    ...
    # Generate gradients
    gpos, gneg, g1, g2, g3 = grad_ss(kz_area, nsamp, sg.SS_VERSE_FRAC, sg.SS_MXG, sg.SS_MXS, sg.SS_TS, sg.SS_EQUAL_LOBES)
    ng1 = len(g1)
    ng2 = len(g2)
    ng3 = len(g3)
    ...
    if status == 'Solved':
        ...
        if sg.SS_SLR_FLAG == 1:
            ...
            if sg.SS_SPECT_CORRECT_FLAG:
                ...
                # %
                print('Note: Spectral Correction for SLR pulses only corrects base bandwidth (not aliased bands)')
                ...
                for idx in range(nZ2):
                    for bidx in range(oversamp_slr):
                        s_rfm[idx, bidx::oversamp_slr] = b2rf(bsf[idx] * s_bi[bidx::oversamp_slr])
                
                # % Now calculate new z beta polynomial by inverse FFT, then use
                # % SLR to get RF
                # %
                z_bm = fftr(np.sin(s_rfm / 2), z_np, 0) #  Each row now scaled by tip-angle
                ...
                for idx in range(len(s_b)):
                    for zidx in range(z_np):
                        idx_intrp = int(1 + ((idx - 1) * Nper + zidx - 1) / Ntotal * len(s_bi))
                        tmp_z_rf = np.interp(range(z_rf.shape[1]), range(z_rf.shape[1]), z_rf[zidx, :].real, idx_intrp, 'spline')
                ...
                # % SLR to get RF
                # %
                z_bm = fftr(np.sin(s_rfm / 2), z_np, 1) #  Each row now scaled by tip-angle
                z_rfm = []
                if dbg:
                    print('Doing SLR in Z...  ')
            ...
            if sg.SS_VERSE_B1:
                ...
                z_rfvmax, g2v = ss_b1verse(g2, z_rfvmax1, sg.SS_MAX_B1, sg.SS_MXG, sg.SS_MXS, sg.SS_TS, sg.SS_GAMMA, sg.SS_SLEW_PENALTY, dbg)
            ...
            if sg.SS_SPECT_CORRECT_FLAG:
                ...
                s_rfm = ss_spect_correct(s_b, bsf, Nper, Noff, (f - f_off) / (fs / 2), ptype, 'Flyback', sg.SS_SLR_FLAG, sg.SS_SPECT_CORRECT_REGULARIZATION, dbg)
            else:
                s_rfm = ang * np.conj(s_b)[:, None] * np.ones((len(s_b), ng2), dtype=np.complex128)
            ...
            z_bmod = z_b * np.exp(1j * 2 * np.pi * np.arange(z_np) * sg.SS_TS * fpass_mid)
            ...
            # % Modulate back
            # %
            z_bv = z_bvmod * np.exp(-1j * 2 * np.pi* np.arange(z_np)[:, None] * sg.SS_TS * fpass_mid)
            z_bv = z_bv.squeeze()
