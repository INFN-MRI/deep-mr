...
__all__ = [
    "make_trapezoid",
    "make_cartesian_gradient",
    "make_crusher",
    "make_arbitrary_gradient",
    "compose_gradients",
    "derivative",
    "integration",
]
    ...
    if np.abs(area) > 0:
        ...
        if rampsamp:
            ...
            if triareamax > np.abs(area):
                ...
                # trapezoid pulse
                nflat = int(
                    np.ceil(abs(area - triareamax) / gamma / gmax / gdt / 2) * 2
                )
                ramp_up = np.linspace(0, ramppts, num=ramppts + 1) / ramppts
                ramp_dn = np.linspace(ramppts, 0, num=ramppts + 1) / ramppts
...
def make_cartesian_gradient(k, gmax, smax, gdt, rew_derate, fid=(0, 0), flyback=False):
    ...
    if gmax is not None and smax is not None:
        ...
        if kmin == 0:
            grad, ramppts = _min_trap_grad(
                kmax, gmax * 1e-3, smax, gdt * 1e-6, k.shape[0]
            )  # noqa
        else:
            grad, ramppts = _min_trap_grad(
                2 * kmax, gmax * 1e-3, smax, gdt * 1e-6, k.shape[0]
            )  # noqa
        ...
        if flyback:
            if kmin == 0:
                gfly, _ = make_trapezoid(
                    -kmax, gmax, rew_derate * smax, gdt, rampsamp=True
                )  # noqa
            else:
                gfly, _ = make_trapezoid(
                    -2 * kmax, gmax, rew_derate * smax, gdt, rampsamp=True
                )  # noqa
        else:
        ...
        if kmin == 0:
            ...
            gpre, _ = make_trapezoid(
                -triarea, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
            )  # noqa
        else:
            gpre, _ = make_trapezoid(
                -kmax, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
            )  # noqa
        ...
        if act_gmax > gmax:
            warnings.warn(
                f"actual gmax = {act_gmax} [mT/m] exceed maximum gradient = {gmax} [mT/m]"
            )
        ...
        if act_smax > smax:
            warnings.warn(
                f"actual smax = {act_smax} [T/m/s] exceed maximum slew rate = {smax} [T/m/s]"
            )
...
def make_arbitrary_gradient(k, gmax, smax, gdt, rew_derate, fid=(0, 0), balance=False):
    ...
    if gmax is not None and smax is not None:
        # calculate waveform
        k, grad = _min_time_gradient(
            k.T, 0.9 * gmax, 0.8 * smax, gdt, g_start=0.0, g_end=0.0
        )  # noqa
        ...
        if balance:
            # rewinder
            grewx, _ = make_trapezoid(
                -k[0][-1], 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
            )  # noqa
            grewy, _ = make_trapezoid(
                -k[1][-1], 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
            )  # noqa
...
def _make_ramp(
    g_start: float, g_end: float, smax: float, gdt: float
...
def _min_time_gradient(
    ...
    interp1 = interpolate.CubicSpline(
        s_of_p, np.linspace(0, num_p - 1, int((num_p - 1) / dp + 1))
    )
    ...
    # compute time
    t_of_s = integration(1.0 / st_of_s, ds)
