...
def make_projection(ordering, traj, shape, accel, rotargs):
    ...
    phi = make_tilt(
        rotargs["tilt_type"][0], nintl + int(dummy), Rplane, nframes, 1, False
    )
    nphi = phi.shape[0]
    if dummy:
        phi[:nframes] = 0.0
    ...
    if ordering == "sequential":  # all the planes, then move to next interleave
        ...
        "shuffl" in ordering
    ):  # planes have the correct length already, expand interleave rotation
...
def make_stack(ordering, traj, kz, plan, kwargs):
    ...
    if (
        tilt
    ):  # phi, kr, kecho, kt already account for nslices, I only have to expand plan and kz
