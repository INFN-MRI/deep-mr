...
__all__ = ["config_cartesian_2D", "config_cartesian_3D", "config_stack", "config_projection"]
...
def config_cartesian_2D(fov, shape, accel, kwargs):
    ...
    return [FOVx, FOVy], [read_shape, phase_shape], [Ry, pf], [readargs, phaseargs], ordering
...
def config_cartesian_3D(fov, shape, accel, kwargs):
    ...
    return [FOVx, [FOVy, FOVz]], [read_shape, phase_shape], [Ry, Rz, pf], [readargs, phaseargs], ordering
...
def config_stack(fov, shape, accel, kwargs):
    ...
    if "acs_shape" in kwargs:
        ...
        if np.isscalar(acs_shape):
            acs_shape = [acs_shape]
        if accel[1] != 1:
            assert len(acs_shape) == 2, "Please provide ACS shape both for in-plane and z directions."
        else:
    ...
    return [FOVr, FOVz], [read_shape, phase_shape], [Rplane, [Rz, pf]], [planeargs, zargs], ordering
...
def config_projection(fov, shape, accel, kwargs):
    ...
    # ordering
    if "ordering" in kwargs: # allowed are (sequential, interleaved, shuffled, multiaxis-shuffled)
        ordering = kwargs["ordering"]
    ...
    if "tilt_type" in kwargs:
        ...
        if isinstance(tilt_type, (list, tuple)) is False:
            tilt_type = [tilt_type, "uniform"] #  assume provided order is for through-plane rotation    
        # assign to rotations
    ...
    return fov, [npix, [nplanes, nechoes, nframes]], accel, [planeargs, rotargs], ordering