...
def prep_1d_phase_plan(fov, shape, accel, ordering, **kwargs):
    """ """
    # parse parameters
    gmax, smax, gdt, rew_derate, fid, acs_shape, _, _ = utils.get_cartesian_defaults(
        kwargs
    )
    ...
    # actual design
    if gmax is not None:
        gpre, _ = utils.make_trapezoid(
            -kzmax, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
        )  # noqa
    else:
        gpre = None
...
def prep_2d_phase_plan(fov, shape, accel, ordering, **kwargs):
    ...
    if gmax is not None:
        gpre, _ = utils.make_trapezoid(
            -kymax, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
        )  # noqa
    else:
        gpre = None
    ...
    if nframes == 1:
        ...
        mask = utils.parallel_imaging(
            3, [nframes, ny], [1, int(Ry)], acs_shape, shift, crop_corner=False
        )
        if pf != 1.0:
            mask *= utils.partial_fourier(ny, pf)
        ...
        if ordering == "interleaved":
            ...
            raise RuntimeError(
                f"ordering = {ordering} not recognized - set either 'sequential' or 'interleaved'"
            )
...
def prep_3d_phase_plan(fov, shape, accel, accel_type, ordering, **kwargs):
    ...
    assert accel_type in [
        "PI",
        "CS",
    ], f"Acceleration type = {accel_type} not recognized. Choose either 'PI' or 'CS'."
    if nframes > 1 and accel_type == "PI":
        warnings.warn(
            "Acceleration type was set to 'PI', but nframes is > 1. We only support 'CS' for 3D kt acquisition for now. Switching to 'CS'"
        )
        accel_type = "CS"
    ...
    # actual design
    if gmax is not None:
        gprey, _ = utils.make_trapezoid(
            -kymax, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
        )  # noqa
        gprez, _ = utils.make_trapezoid(
            -kzmax, 0.9 * gmax, rew_derate * smax, gdt, rampsamp=True
        )  # noqa
    ...
    if accel_type == "PI":
        mask = utils.parallel_imaging(
            3, [ny, nz], [int(Ry), int(Rz)], acs_shape, shift, crop_corner=True
        )
        ...
        if Ry * Rz > 1:
            ...
            for f in range(nframes):
                tmp, _ = utils.poisson_disk(
                    [ny, nz, 1], Ry * Rz, acs_shape, crop_corner=True, seed=f
                )
                mask.append(tmp)
