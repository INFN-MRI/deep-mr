...
def radial(fov, shape, accel=1, nspokes=None, osf=1.0, **kwargs):
    ...
    """
    # parse defaults
    gmax, smax, gdt, rew_derate, fid, acs_shape, flyback, _ = utils.get_cartesian_defaults(kwargs)
    
    # build base interleaf and rotation angles
    kr, phi, shape, acs_shape, variant = _make_radial_interleave(fov, shape, accel, nspokes, osf, kwargs)
    ...
    grad, adc, echo_idx = _make_radial_gradient(kr, gmax, smax, gdt, rew_derate, fid, flyback)
...
def _make_radial_gradient(kr, gmax, smax, gdt, rew_derate, fid, flyback):
    ...
    # build gradient
    grad, adc, echo_idx = utils.make_cartesian_gradient(kr, gmax, smax, gdt, rew_derate, fid, flyback)
