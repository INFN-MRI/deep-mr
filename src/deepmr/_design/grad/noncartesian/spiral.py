...
def spiral(fov, shape, accel=1, nintl=1, **kwargs):
    ...
    (
        ...
        acs_shape,
        moco_shape,
    ) = utils.get_noncart_defaults(kwargs)

    # build base interleaf and rotation angles
    kr, phi, shape, acs_shape, variant = _make_spiral_interleave(
        fov, shape, accel, nintl, kwargs
    )
    ...
    kr, grad, adc, _ = utils.make_arbitrary_gradient(
        kr, gmax, smax, gdt, rew_derate, balance=True
    )
...
def _make_spiral_interleave(fov, shape, accel, nintl, kwargs):
    ...
    # check acceleration
    assert (
        accel > 0 and accel <= nintl
    ), f"Acceleration = {accel} must be between positive and <= {nintl}."
    ...
    if len(r) == 0:
        ...
        assert (
            r.max() + trans_dur < 1.0
        ), "Error! Transition band must finish before kmax is reached."
...
def _postprocess_spiral(variant, k, grad, adc, fid):
    ...
    if grad is not None:
        if variant == "reverse":
            ...
            grad["pre"] = np.flip(grad["rew"], axis=-1)
            grad["read"] = np.concatenate(
                (-np.flip(grad["read"]), grad["read"]), axis=-1
            )
