"""3D Stack-Of-Stars trajectory design routine."""
...
def radial_stack(fov, shape, accel=1, nspokes=None, osf=1.0, **kwargs):
    r"""
    Design a stack of radial trajectories.
    ...
    Args:
        ...
        accel (tuple of ints): acceleration (Rplane, Rz, Pf). Ranges from (1, 1, 1) (fully sampled) to (nspokes, nz, 0.75).
    ...
    traj, grad = radial(fov[0], shape[0], accel[0], nspokes, osf, **kwargs[0])
    ...
    # put together
    traj, plan = utils.make_stack(ordering, traj["compressed"], act_traj["kz"], plan, kwargs[1])
