...
def cartesian2D(fov, shape, accel=1, osf=1.0, **kwargs):
    ...
    """
    # parsing
    fov, shape, accel, kwargs, ordering = utils.config_cartesian_2D(
        fov, shape, accel, kwargs
    )

    # prepare phase encoding plan
    act_traj, gpre, plan = utils.prep_2d_phase_plan(
        fov[1], shape[1], accel, ordering, **kwargs[1]
    )
    ...
    grad["pre"], delta_te = utils.compose_gradients(
        "before", gx=grad["pre"], gy=gpre, **kwargs[0]
    )
    grad["pre"] = grad["pre"][:2, :]
...
def cartesian3D(fov, shape, accel=1, osf=1.0, accel_type="PI", **kwargs):
    ...
    """
    # parsing
    fov, shape, accel, kwargs, ordering = utils.config_cartesian_3D(
        fov, shape, accel, kwargs
    )

    # prepare phase encoding plan
    act_traj, gpre, plan = utils.prep_3d_phase_plan(
        fov[1], shape[1], accel, accel_type, ordering, **kwargs[1]
    )
    ...
    traj, plan = utils.make_multiecho(traj, plan, nechoes=shape[0][1])
    grad["pre"], delta_te = utils.compose_gradients(
        "before", gx=grad["pre"], gy=gpre[0], gz=gpre[1], **kwargs[0]
    )
    grad["rew"] = -np.flip(grad["pre"], axis=-1)
    grad["amp"] = plan
...
def _make_cartesian_readout(fov, shape, osf, kwargs):
    # parse parameters
    gmax, smax, gdt, rew_derate, fid, _, flyback, _ = utils.get_cartesian_defaults(
        kwargs
    )
    ...
    grad, adc, echo_idx = utils.make_cartesian_gradient(
        k, gmax, smax, gdt, rew_derate, fid, flyback
    )
