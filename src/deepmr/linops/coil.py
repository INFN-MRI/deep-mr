...
class CoilOp(dinv.physics.LinearPhysics):
    ...
    * 3D MRI: ``(nsets, ncoils, ..., nz, ny, nx)``
    ...
    def A_adjoint(self, y):
        ...
        if self._ndim == 2:
            x = tmp.sum(axis=1, keepdim=True).sum(axis=2, keepdim=True) # sum over sets and channels
        if self._ndim == 2:
            x = tmp.sum(axis=0, keepdim=True).sum(axis=1, keepdim=True) # sum over sets and channels
