...
def WaveletPrior(ndim, wv="db4", device=None, p=1, level=3, *args, **kwargs):
    ...
    """
    return PnP(
        denoiser=ComplexWaveletDenoiser(ndim, wv, device, p, level, *args, **kwargs)
    )
...
class ComplexWaveletDenoiser(torch.nn.Module):
    def __init__(self, ndim, wv, device, p, level, *args, **kwargs):
        super().__init__()
        self.denoiser = _WaveletPrior(
            level=level, wv=wv, p=p, device=device, wvdim=ndim, *args, **kwargs
        )
    ...
    def forward(self, input, ths):
        ...
        # apply denoising
        output = self.denoiser.prox(input[:, None, ...].to(device), ths).to(
            idevice
