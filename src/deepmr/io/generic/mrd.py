...
def read_mrd(filepath, external=False):
    ...
    with ismrmrd.File(filepath) as file:
        ...
        # get adc
        adc = (
            acquisitions[0]["head"]["discard_pre"],
            npts - acquisitions[0]["head"]["discard_post"],
        )
        ...
        # get slice profile
        tmp = mrd._find_in_user_params(
            mrdhead.userParameters.userParameterString, "slice_profile"
        )
        if tmp is not None:
            head.user["slice_profile"] = mrd._bytes_to_numpy(
                tmp["slice_profile"]
            ).astype(np.float32)

        # get basis
        tmp = mrd._find_in_user_params(
            mrdhead.userParameters.userParameterString, "basis"
        )
        if tmp is not None:
        ...
        # get separability
        tmp = mrd._find_in_user_params(
            mrdhead.userParameters.userParameterString, "separable"
        )
        if tmp is not None:
        ...
        # get mode
        tmp = mrd._find_in_user_params(
            mrdhead.userParameters.userParameterString, "mode"
        )
        if tmp is not None:
            mode = tmp["mode"]
            head.user["mode"] = mode
...
def _sort_data(data, trajdcf, acquisitions, mrdhead):
    ...
    # order
    icontrast = np.asarray(
        [acquisitions[n]["head"]["idx"]["contrast"] for n in range(nacq)]
    )
    iz = np.asarray([acquisitions[n]["head"]["idx"]["slice"] for n in range(nacq)])
    iview = np.asarray(
        [acquisitions[n]["head"]["idx"]["kspace_encode_step_1"] for n in range(nacq)]
    )
    ...
    if trajdcf is not None:
        ...
        trajdcf = trajdcftmp
        traj, dcf = trajdcf[:, 0, ..., :-1], trajdcf[:, 0, ..., -1]
    else:
        # actual sorting
        traj, dcf = None, None
