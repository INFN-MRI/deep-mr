...
def grappaop(calib: torch.Tensor, ndim: int = 2, lamda: float = 0.01) -> SimpleNamespace:
    """
...
def _grappa_op_2d(calib, lamda):
    ...
    Gy = torch.linalg.solve(
        _bdot(Syh, Sy) + lamda0[:, None, None] * torch.eye(Syh.shape[1])[None, ...], _bdot(Syh, Ty))
    
    Sxh = Sx.conj().permute(0, 2, 1)
    lamda0 = lamda * torch.linalg.norm(Sxh, dim=(1, 2)) / Sxh.shape[1]
    Gx = torch.linalg.solve(
        _bdot(Sxh, Sx) + lamda0[:, None, None] * torch.eye(Sxh.shape[1])[None, ...], _bdot(Sxh, Tx))
...
def _grappa_op_3d(calib, lamda):
    ...
    Gz = torch.linalg.solve(
        Szh @ Sz + lamda0 * torch.eye(Szh.shape[0]), Szh @ Tz)
        
    Syh = Sy.conj().permute(1, 0)
    lamda0 = lamda * torch.linalg.norm(Syh) / Syh.shape[0]
    Gy = torch.linalg.solve(
        Syh @ Sy + lamda0 * torch.eye(Syh.shape[0]), Syh @ Ty)
    
    Sxh = Sx.conj().permute(1, 0)
    lamda0 = lamda * torch.linalg.norm(Sxh) / Sxh.shape[0]
    Gx = torch.linalg.solve(
        Sxh @ Sx + lamda0 * torch.eye(Sxh.shape[0]), Sxh @ Tx)
