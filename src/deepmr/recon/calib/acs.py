...
def find_acs(data, shape, coord=None, dcf=None):
    """
    ...
    data : np.ndarray | torch.Tensor
        Input k-space data.
    shape : int
        (Maximum) size of the calibration region.
    ...
    * **3Dnoncart:** ``(ncoils, ncontrasts, nviews, nsamples)``.
    ...
    """
    if coord is None:
        return _find_cart_acs(data, shape)
    else:
        return _find_noncart_acs(data, shape, coord, dcf)
    
    
# %% local utils
def _find_cart_acs(kspace, shape):
    ...
    while (all(l > 0 and r < mask.shape[ii] for ii, (l, r) in enumerate(slices)) and  np.all(mask[tuple([slice(l-1, r+1) for l, r in slices])])):
    ...
    kspace = np.moveaxis(kspace[tuple([slice(l0, r0) for l0, r0 in slices] + [slice(None)])].copy(), -1, coil_axis)
    
    # cast back to torch if required
    if istorch:
        kspace = torch.as_tensor(kspace, device=device, dtype=torch.complex64)
    
    # restrict to maximum shape
    center = np.asarray(kspace.shape[-2], dtype=int) // 2
...
def _find_noncart_acs(kspace, cal_shape, coord, dcf):
    ...
    kspace = kspace.reshape(*kspace.shape[:2], 1, -1, kspace.shape[-1]) # (nslices, ncoils, 1, ncontrasts * nviews, nsamples)
    coord = coord.reshape(1, -1, *coord.shape[-2:]) # (1, ncontrasts * nviews, nsamples, ndim)
    if dcf is not None:
        dcf = dcf.reshape(1, -1, dcf.shape[-1])
    
    # find subset of coord along nsamples dim
    cabs = (coord**2).sum(axis=-1)**0.5 # sqrt(kx**2 + ky**2 + kz**2)
    ind = cabs[0, 0] <= cal_shape // 2 # from (-mtx /2, mtx / 2) to (-cal / 2, cal / 2)
    ...
    # grid calibration
    cal_ksp = _signal.gridding(cal_ksp, cal_coord, cal_shape).squeeze() # (nslices, ncoils, ny, nx) or (ncoils, nz, ny, nx)
